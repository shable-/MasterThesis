...
ar_f1 = np.load('/import/sun-data/hable/epsCC_30/F025_1/averages2/arrays/average_F025_1_ZNE.npy')
ar_f2 = np.load('/import/sun-data/hable/epsCC_30/F05_2/averages2/arrays/average_F05_2_ZNE.npy')
ar_f3 = np.load('/import/sun-data/hable/epsCC_30/F1_4/averages2/arrays/average_F1_4_ZNE.npy')
...
ax1 = plt.gca()
ax1.plot(date[64:], botheps_f1[64:], c='Orange', lw=1.8, label='0.25-1.0 Hz', zorder=200)
print(len(np.arange(1,81,1)))
print(len(botheps_f1[64:]))
f1_s = np.polyfit(np.log(np.arange(1,81,1)), (-botheps_f1[64:])+0.2, 1)
log_f1 = f1_s[1] + np.log(np.arange(1,81,1))*f1_s[0]
print(log_f1)
ax1.plot(date[64:], -log_f1+0.2, c='k', lw=1.8, zorder=200)

ax1.fill_between(date, botheps_f1-rms1, botheps_f1+rms1, alpha=0.2, facecolor='Orange', edgecolor='Orange')
ax1.plot(date, botheps_f2, c='OrangeRed', lw=1.8, label='0.5-2.0 Hz', zorder=200)
ax1.fill_between(date, botheps_f2-rms2, botheps_f2+rms2, alpha=0.2, facecolor='OrangeRed', edgecolor='OrangeRed')
ax1.plot(date[64:-1], botheps_f3[64:], c='FireBrick', lw=1.8, label='1.0-4.0 Hz', zorder=200)
f3_s = np.polyfit(np.log(np.arange(1,80,1)), (-botheps_f3[64:])+0.2, 1)
log_f3 = f3_s[1] + np.log(np.arange(1,80,1))*f3_s[0]
print(log_f1)
ax1.plot(date[64:-1], -log_f3+0.2, c='k', lw=1.8, zorder=200)

ax1.fill_between(date[0:-1], botheps_f3-rms3, botheps_f3+rms3, alpha=0.2, facecolor='FireBrick', edgecolor='FireBrick')
ax1.legend(loc=4, prop={'size':12})

ax1.set_ylabel('$\Delta$v/v (%) \n', fontsize = 12)
fig.add_subplot(2,1,2)
ax2 = plt.gca()
ax2.plot(date, bothCC_f1, c='Orange', lw=1.8, label='0.25-1.0 Hz')
ax2.plot(date, bothCC_f2, c='OrangeRed', lw=1.8, label='0.5-2.0 Hz')
ax2.plot(date[0:-1], bothCC_f3, c='FireBrick', lw=1.8, label='1.0-4.0 Hz')
...
#plt.savefig('/import/sun-data/hable/posterplots_new/ZNE_in_one.pdf')
plt.show()
plt.close()
